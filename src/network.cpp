...
void convolution(const Weight *weight, const pBox *pbox, pBox *outpBox, const struct pBox *matrix){
    ...
    if(weight->pad==0){
		...
		matrixXmatrix(matrix->pdata, weight->pdata, matrix->width, matrix->height,
			weight->kernelSize*weight->kernelSize*weight->lastChannel, weight->selfChannel, outpBox->pdata);
	}
    else{
...
void vectorXmatrix(float *matrix, float *v,int size, int v_w, int v_h, float *p) {
	/*
	vector<vector<float>> arrA(1);
	vector<vector<float>> arrB(v_w);
	vector<vector<float>>  res(1);
	for (int i = 0; i < 1; i++) {
		arrA[i].resize(v_w);
	}
	for (int i = 0; i < v_w; i++) {
	...
	for (long i = 0; i < 1; i++) {
		for (long j = 0; j < v_w; j++) {
			arrA[i][j] = matrix[i * v_w + j];
		}
	}
	for (long i = 0; i < v_h; i++) {
		for (long j = 0; j < v_w; j++) {
			arrB[j][i] = v[i * v_w + j];
		}
	}
	...
	long rowA = arrA.size();
	//矩阵arrA的列数  
	long colA = arrA[0].size();
	//矩阵arrB的行数  
	long rowB = arrB.size();
	//矩阵arrB的列数  
	long colB = arrB[0].size();
	//相乘后的结果矩阵  
	if (colA != rowB)//如果矩阵arrA的列数不等于矩阵arrB的行数。则返回空  
	{
		return;
	...
	else
	{
		...
		for (long i = 0; i < rowA; ++i)
		{
			for (long j = 0; j < colB; ++j)
			{
				for (long k = 0; k < colA; ++k)
				{
					res[i][j] += arrA[i][k] * arrB[k][j];
					//res[i][j] += arrA[i][k] * arrB[j][k];
				}
